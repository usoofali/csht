...
$("#add_data").on("submit", function (event) {
  ...
  try {
    $.ajax({
      ...
      contentType: false,
      cache: false,
      url: "ajax/super/student_add_ajax.php",
      data: formData,
...
$("#edit_data").on("submit", function (event) {
  ...
  $.ajax({
    type: "POST",
    url: "ajax/super/student_edit_ajax.php",
    data: parametros,
    beforeSend: function (objeto) {
      // Display a loading message using SweetAlert before sending the AJAX request
      Swal.fire({
...
$("#branch").on("change", function (event) {
  ...
  $.ajax({
    type: "POST",
    url: "ajax/get_dept_session_ajax.php",
    data: { id: branch },
    success: function (data) {
      data = JSON.parse(data);
      dept.innerHTML = '';
      ...
      dept.appendChild(option); // Append <option> to the <select> element
      for (var key in data["dept"]) {
        if (data["dept"].hasOwnProperty(key)) {
          var option = document.createElement("option");
          option.value = key; // Set the value attribute of <option> to the key
          option.textContent = data["dept"][key]; // Set the text content of <option> to the value
          dept.appendChild(option); // Append <option> to the <select> element
        }
      }
      var option = document.createElement("option");
      option.value = ""; // Set the value attribute of <option> to the key
      option.textContent = "Select Session"; // Set the text content of <option> to the value
      session.appendChild(option);
      for (var key in data["session"]) {
        var option = document.createElement("option");
        option.value = key; // Set the value attribute of <option> to the key
        option.textContent = data["session"][key]; // Set the text content of <option> to the value
        session.appendChild(option); // Append <option> to the <select> element
      }
    },
  });
});
